...
class RomanToInteger {
    ...
    public int romanToInt(String s) {
        ...
        for (int i=0 ; i < array.length ; i++) {
          if ((i <= array.length - 2) && this.hashMap.get(array[i]) < this.hashMap.get(array[i + 1])) {
                number += this.hashMap.get(array[i + 1]) - this.hashMap.get(array[i]);
                i += 1;
            } else {
                number += this.hashMap.get(array[i]);
            }
        }
